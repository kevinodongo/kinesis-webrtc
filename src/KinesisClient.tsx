...
export class KinesisClient {
    ...
    */
    public async getMedia(constraints: any, include = false) {
        const stream: any = await navigator.mediaDevices.getUserMedia(constraints).catch(err => handleError(err))
   ...
   */
    ...
    */
    public async setKinesisClient(value: KinesisValue) {
        ...
        const getIceServer: any =
            await kinesisChannelsClient
                .getIceServerConfig({
                    ChannelARN: this.channelARN,
                })
                .promise();
   ...
   */
    public async masterConnect() {
        this.signalingClient.on("sdpOffer", async (offer: any, remoteClientId: any) => {
            const configuration = {
            ...
            this.peerConnection.addEventListener("icecandidate", ({ candidate }: any) => {
                if (candidate) {
                    var canTrickle = this.peerConnection.canTrickleIceCandidates;
                    if (canTrickle) {
        ...
        this.signalingClient.on(
            ...
            async (candidate: any, remoteClientId: any) => {
                this.peerConnection =
                    this.peerConnectionByClientId[remoteClientId];
